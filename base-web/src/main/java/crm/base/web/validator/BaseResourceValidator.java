package crm.base.web.validator;
...
public class BaseResourceValidator {
  ...
  protected void validateNullable(
      @Nullable String string, int minLength, int maxLength, int code, @NotNull String message) {
    if (!StringUtils.isEmpty(string) && (string.length() < minLength || string.length() > maxLength)) {
      throw new BadRequestWebException(message, code);
    }
  }

  protected void validateNotBlank(@Nullable String string, int code, @NotNull String message) {
