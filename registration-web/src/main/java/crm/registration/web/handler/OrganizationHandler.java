...
import crm.registration.web.resources.AuthenticationOutResource;
...
public class OrganizationHandler extends BaseRegistrationHandler {
  ...
  public @NotNull Mono<ServerResponse> register(@NotNull ServerRequest request) {
    return request
        .bodyToMono(OrganizationRegisterInResource.class)
  ...
  private @NotNull Mono<EmailConfirmation> findEmailConfirmation(@NotNull OrganizationRegisterInResource resource) {
    return emailConfirmationDao.findByEmailAndCode(resource.email(), resource.activationCode());
  }
  ...
  private @NotNull Mono<AuthenticationOutResource> createOrganization(
      @NotNull OrganizationRegisterInResource resource, @NotNull Country country) {
    return organizationDao
        .create(resource.orgName(), country)
        .onErrorResume(throwable -> Mono.error(ExceptionUtils.toBadRequest(
            throwable,
  ...
  private @NotNull Mono<AuthenticationOutResource> createOrgAdmin(
      @NotNull OrganizationRegisterInResource resource, @NotNull Organization organization) {
    ...
    return createOrgAdmin(resource, organization, salt, hash)
        .onErrorResume(throwable -> organizationDao
  ...
  private @NotNull Mono<User> createOrgAdmin(
