...
import static crm.registration.web.exception.RegistrationErrors.INVALID_ACCESS_ROLE;
import static crm.registration.web.exception.RegistrationErrors.INVALID_ACCESS_ROLE_MESSAGE;
...
import crm.registration.web.exception.RegistrationErrors;
import crm.registration.web.resources.*;
...
public class UserGroupHandler extends BaseRegistrationHandler {
  ...
  public static final AccessRole[] USER_GROUP_AND_USERS_VIEWERS = ArrayUtils
      .combineUniq(USER_GROUP_VIEWERS, UserHandler.USER_VIEWERS);
  ...
  public @NotNull Mono<ServerResponse> findPage(@NotNull ServerRequest request) {
    return RequestUtils
        ...
        .flatMap(authorized -> {
          ...
          return userGroupDao.findPageByOrganization(
              pageRequest.offset(),
  ...
  public @NotNull Mono<ServerResponse> findUsersPage(@NotNull ServerRequest request) {
    return RequestUtils
        ...
        .flatMap(authorized -> {
          var pageRequest = authorized.getParam();
          return minimalUserDao.findPageByOrganizationAndGroup(
              pageRequest.offset(),
              pageRequest.pageSize(),
              authorized.getOrgId(),
              pageRequest.id());
