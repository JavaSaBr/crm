...
public class AuthenticationHandler {
  ...
  public @NotNull Mono<ServerResponse> authenticate(@NotNull ServerRequest request) {
    return request
        .bodyToMono(AuthenticationInResource.class)
  ...
  public @NotNull Mono<ServerResponse> authenticateByToken(@NotNull ServerRequest request) {
    return Mono
        .fromSupplier(() -> request.pathVariable("token"))
  ...
  private @NotNull AuthenticationOutResource authenticate(@NotNull AuthenticationInResource resource, @NotNull User user) {
  ...
  private @NotNull Mono<User> loadUserByLogin(@NotNull AuthenticationInResource resource) {
    var login = resource.login();
    return StringUtils.isEmail(login) ? userDao.findByEmail(login) : userDao.findByPhoneNumber(login);
  }
}
