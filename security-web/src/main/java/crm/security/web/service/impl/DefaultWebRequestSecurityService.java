...
import static crm.security.web.exception.SecurityErrors.*;
...
public class DefaultWebRequestSecurityService implements WebRequestSecurityService {
  ...
  @Override
  public @NotNull Mono<User> isAuthorized(@NotNull ServerRequest request, @NotNull AccessRole accessRole) {
    return tokenService
        .findUserIfNotExpired(request
            .headers()
            .header(HEADER_TOKEN)
            .stream()
            .findFirst()
  ...
  @Override
  public @NotNull Mono<User> isAuthorized(@NotNull ServerRequest request, @NotNull AccessRole... accessRoles) {
  ...
  @Override
  public @NotNull Mono<User> isAuthorized(@NotNull ServerRequest request) {
    return tokenService.findUserIfNotExpired(request
        .headers()
        .header(HEADER_TOKEN)
        .stream()
        .findFirst()
  ...
  private void resolveAllRoles(@NotNull Set<AccessRole> ownedRoles, @NotNull Set<AccessRole> roles) {
    ...
    for (var role : roles) {
      ownedRoles.add(role);
      Collections.addAll(ownedRoles, role.subRoles());
    }
  }
